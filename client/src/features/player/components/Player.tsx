...
export const Player = () => {
  ...
  const currentSongId = useAppSelector((state: RootState) => state.player.queue[0]);
      ...
      <div className="fixed bottom-0 w-full h-32 bg-gray-200 flex items-center justify-between px-4">
        <div className="flex">
          ...
          <div className="-mt-1">
            ...
            <p className="text-slate-400">{ formatArtists(currentSongInfo?.artist)}</p>
          </div>
        </div>
        <div className="flex items-center">
          ...
          <div className="mx-4">
            { isPlaying
              ? <RiPauseCircleFill size={48} style={{ cursor: "pointer" }} onClick={() => onPlayPressed()}/>
              : <RiPlayCircleFill size={48} style={{ cursor: "pointer" }} onClick={() => onPlayPressed()}/>
            }
          </div>
          <RiSkipForwardFill size={32} style={{ cursor: "pointer" }} />
        </div>
      </div>
      <div className="fixed bottom-0 bg-gray-600 h-2 w-full">
      </div>
    </div>
  )
}
